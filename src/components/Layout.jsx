...
function Sidebar({ onOpenCmd, className = '' }) {
  ...
  return (
    <div
      className={cn(
        'w-64 bg-gray-50 border-r border-gray-200 flex flex-col h-full shrink-0',
        className,
      )}
    >
      ...
      <div className="p-3">
        ...
        >
          <Search size={14} />
          <span>搜尋...</span>
          <kbd className="ml-auto text-[10px] bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 font-mono text-gray-400">
            ⌘K
          </kbd>
        </button>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 px-1">
            {tags.map(([tag]) => (
              <span
                key={tag}
                className="px-2 py-1 bg-white border border-gray-200 rounded-md text-xs text-gray-600 hover:border-blue-300 cursor-pointer"
              >
                #{tag}
              </span>
            ))}
      ...
      <div className="p-4 border-t bg-gray-50/50">
        <button
          onClick={logout}
          className="flex items-center gap-2 text-gray-500 text-sm w-full hover:bg-red-50 hover:text-red-600 p-2 rounded transition-colors"
        >
          <LogOut size={16} /> 登出
        </button>
      </div>
  ...
  return (
    <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md border-t border-gray-200 z-50 pb-[env(safe-area-inset-bottom)]">
      <div className="flex justify-around py-2">
        <button
          onClick={() => navigate('/')}
          className={`flex flex-col items-center gap-0.5 p-1.5 min-w-[56px] ${isActive('/') ? 'text-blue-600' : 'text-gray-400'}`}
        >
          <HomeIcon size={20} />
        ...
        <button
          onClick={onOpenCmd}
          className="flex flex-col items-center gap-0.5 p-1.5 min-w-[56px] text-gray-400 active:text-gray-900"
        >
          <Search size={20} />
        ...
        <button
          onClick={() => navigate('/browse')}
          className={`flex flex-col items-center gap-0.5 p-1.5 min-w-[56px] ${isActivePrefix('/browse') ? 'text-blue-600' : 'text-gray-400'}`}
        >
          <FolderOpen size={20} />
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-label="搜尋筆記"
        className="w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200 animate-in fade-in zoom-in-95 duration-200"
      >
